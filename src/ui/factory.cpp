...
std::expected<farcical::ui::Menu*, farcical::engine::Error> farcical::ui::factory::CreateMenu(
    ...
    if(!config.id.empty()) {
        ...
        const TextureProperties& buttonProperties{config.buttonConfig.textureProperties[static_cast<int>(Button::Status::Normal)].second};
        ...
        const sf::Vector2u& textureSize{sf::Vector2u{
        ...
        for(unsigned int itemIndex = 0; itemIndex < menuItems.ids.size(); ++itemIndex) {
            ...
            if(!createItemResult.has_value()) {
                const std::string failMsg{"Error: Failed to create MenuItem (id=" + config.menuItemConfigs[itemIndex].id + ")."};
                return std::unexpected(engine::Error{engine::Error::Signal::InvalidConfiguration, failMsg});
        ...
        const auto& createMenuController{
            currentScene->GetRootContainer().CreateMenuController(menu, game.GetEngine().GetEventSystem(), game.GetEngine().GetInputSystem())
        };
        return menu;
